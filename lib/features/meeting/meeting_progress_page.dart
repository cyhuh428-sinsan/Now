...
class _MeetingProgressPageState extends ConsumerState<MeetingProgressPage> {
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('LLM에 전송 전 내용을 수정할 수 있습니다.',
                style:
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  Navigator.pop(ctx);
                  // 수정된 텍스트로 세그먼트 재구성
                  final editedLines = editController.text
                      .split('\n')
                      .map((l) => l.trim())
                      .where((l) => l.isNotEmpty)
                      .toList();
                  final editedSegments = editedLines
  ...
  Future<void> _extractAndNavigate(
    ...
    _navigateToReview(null);
  }

  void _navigateToReview(List? emptyItems) {
    if (emptyItems != null) {
