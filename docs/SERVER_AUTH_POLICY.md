...
- 모바일 앱과 설치형 프로그램은 서버 주소, API 토큰 또는 사용자별 접속 토큰, 사용자 ID, 기기 ID를 저장해 서버에 연결합니다.
...
- 관리자 화면과 API에서 Web 로그인 비밀번호를 설정할 수 있습니다.
...
- `NOW_USER_TOKEN_REQUIRED=false`인 개인 서버 기본 모드에서는 앱/설치형 데이터 API가 `NOW_API_TOKEN`을 확인합니다.
...
현재의 단일 `NOW_API_TOKEN`만으로는 공용 서버를 정식 오픈하기에 부족합니다. 사용자별 접속 토큰, Web 로그인, Web 세션, 2단계 코드 검증, 데이터 API 검증 단계까지 준비되어 있으며, 공용 서버 오픈 전에는 아래 기능을 추가로 확정해야 합니다.
