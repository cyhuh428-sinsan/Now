...
- 설치형 프로그램을 Web 정적 파일 복사/포장 방식에서 분리하고 `desktop/app`의 별도 HTML/CSS/JS 소스로 전환.
- `desktop/scripts/sync-web-assets.mjs`와 `sync:web` 실행 흐름을 제거하고, `npm run start`와 `npm run dist:win`이 설치형 전용 소스를 직접 사용하도록 수정.
- 설치형 화면은 Web과 같은 어두운 디자인 계열, 사이드바, 열린 탭, 지식 체계, 편집 패널 구조를 유지하되 PC 로컬 문서 중심으로 동작.
- 새 메모와 기존 메모의 제목/본문을 즉시 편집할 수 있도록 설치형 전용 편집 흐름을 구성.
- 설치형 저장 키를 `nownote.desktop.v1`로 분리하고, 예전 포장 화면의 `nownote.web.v1` 데이터가 있으면 첫 실행 때 가져오도록 보강.
