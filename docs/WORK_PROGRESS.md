...
### 설치형 프로그램 Web 동일 화면 재생성

- 설치형 프로그램을 별도 `.exe`로 유지하되 `desktop/app` 화면 소스를 Web 전용 기능을 제외한 Web 동일 디자인/기능 구조로 재생성.
- `desktop/scripts/sync-web-assets.mjs`와 `sync:web` 실행 흐름을 제거하고, `npm run start`와 `npm run dist:win`이 설치형 전용 소스를 직접 사용하도록 수정.
- 설치형 화면에 Web과 동일한 사이드바, 열린 탭, 설정, 서버 연결, 그래프, 속성, Canvas, 빠른 기록, 명령 팔레트, 복구/가져오기, 출판, 작업공간 화면 구조를 포함.
- hosted Web 로그인, 서버 공유문서 전용 조회, 그룹 참가/그룹 메신저는 Web 전용 기능으로 설치형에서 숨김 상태를 유지.
- 새 메모와 기존 메모의 제목/본문을 즉시 편집할 수 있는 Web 동일 편집 흐름을 유지.
- 설치형 저장 키는 Web 동일 데이터 구조와 호환되도록 `nownote.web.v1`을 유지.
- `desktop/README.md`, 루트 README, 1.1 설치형 설계서, Web 표면 검증 스크립트를 독립 설치형 기준으로 갱신.
- `npm run dist:win`으로 `desktop/dist/NowNote-Setup-0.1.0-x64.exe` 재생성 통과.
- `npm run check:storage`로 메모 생성, 제목/본문 수정, 파일 저장, 앱 재시작 후 재로드 자동 점검 통과.

## 2026-06-01 KST

### NowNote 1.9 작업공간과 운영형 지식 관리 시작

- `main`의 1.8 및 그룹 공유 Web 전용 제한 병합 완료 상태를 기준으로 `codex/workspace-ops-1.9` 브랜치 생성.
- `docs/NOW_1_9_WORKSPACE_OPERATIONS_DESIGN.md`를 추가해 1.9 완료 기준 문서로 확정.
- 화면 설정에 작업공간 저장/열기와 지식 건강 점검 패널을 추가.
- 열린 탭, 고정 탭, 선택 메모, 검색어, 그래프 필터, 속성 필터, 화면 상태를 작업공간으로 저장/복원하도록 구현.
- 고립 메모, 오래된 메모, 과도 연결 메모, 속성 누락 메모와 현재 메모 외부 링크를 화면에서 확인하도록 구현.

### 그룹 공유 메모 조회 전용 기능

- 서버의 기존 사용자 그룹(`UserAccount.group_name`)을 사용해 같은 그룹의 다른 사용자 지식 메모를 Web 세션의 pull/list/search 결과에만 포함하도록 구현.
- 그룹 공유 메모는 `tree` 메모만 대상으로 하고, Web에서는 `group:{owner_id}:{local_id}` 표시 ID로 분리해 local_id 충돌을 방지.
- Web에서 그룹 공유 메모는 읽기 전용으로 표시하고 제목/본문/속성/삭제/이동/공유/암호화/하위 추가를 막음.
- 그룹 공유 읽기 전용 메모는 서버 업로드 대상에서 제외해 다른 사용자의 메모를 수정하지 않도록 처리.
- 설치형/모바일/일반 사용자 토큰 동기화에서는 그룹 공유 메모를 내려주지 않고, 자기 계정 메모만 동기화하도록 제한.

### NowNote 1.8 출판/발표/공개 지식 묶음 시작

- `main`의 1.7 병합 완료 상태를 기준으로 `codex/publish-slides-1.8` 브랜치 생성.
- `docs/NOW_1_8_PUBLISH_SLIDES_DESIGN.md`를 추가해 1.8 완료 기준 문서로 확정.
- 설정 화면에 출판/발표 패널을 추가하고 공개 묶음 제목, 설명, permalink, 포함 문서 선택을 화면에서 관리하도록 구현.
- 공유 안 함, 암호화 메모, 공개 제외 속성, 비공개 태그를 공개 산출물에서 자동 제외하도록 구현.
- 공개 전 이메일, 전화번호, 비밀번호/토큰/API key, 주민등록번호 후보를 화면에 표시.
- 공개 HTML 내보내기와 슬라이드 구분선 기반 발표 HTML 내보내기를 추가.
...
# 2026-05-31 02:25 KST

## 작업 내용

- Android 앱 계층메모 화면에서 Web/설치형 암호화 메모 접두어 `NOW_ENCRYPTED_V1:`를 인식하도록 보강.
- 앱은 아직 복호화 기능을 제공하지 않으므로 암호화 메모를 읽기 잠금 상태로 표시.
- 암호화 계층메모의 앱 편집과 서버 분석 요청을 막아 암호문이 평문 편집 과정에서 깨지지 않도록 처리.
- 서버에서 내려온 암호화 계층메모 payload가 앱 DB에 그대로 보존되는 테스트를 추가.

## 검증

- `flutter test` 통과: 60/60.
- release APK 재생성 통과.
- 실제 기기 `R3CN90A1WZF` 설치/실행 점검 통과.

# 2026-05-31 03:35 KST

## 작업 내용

- Android 앱 계층메모 목록에서 제목만 보이고 본문을 읽기 어려운 문제를 수정.
- 계층메모 제목을 누르면 본문 보기 화면이 열리도록 추가.
- 본문 보기 화면에서 평문 메모는 편집으로 바로 이어질 수 있게 하고, 암호화 메모는 복호화된 내용만 읽을 수 있게 분리.
...
# 2026-05-31 03:10 KST

## 작업 내용

- Android 앱에서 Web/설치형과 같은 `NOW_ENCRYPTED_V1` 암호화 메모를 복호화할 수 있도록 Android 네이티브 암호화 채널을 추가.
- Android 네이티브에서 PBKDF2-SHA256 210000회 + AES-GCM 방식으로 암호화/복호화하도록 구현.
- 계층메모 화면에 암호화, 복호화, 잠금, 암호화 해제 동작을 추가.
- 암호화 해제 시 평문으로 저장하고, 서버 연결이 켜져 있으면 동기화를 시도하도록 처리.
- 앱 암호화 서비스 테스트를 추가.
