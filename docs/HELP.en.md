...
On the server-hosted Web login, knowledge notes shared by other users in the same user group are shown as read-only.
Group-shared notes cannot be edited or deleted, are excluded from uploads from your account, and are not shown in desktop or mobile token sync.
