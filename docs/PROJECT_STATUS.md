...
- 모바일 앱: 일자별 메모, 계층 메모, 음성 입력, 서버 연결 설정, 서버 동기화 흐름, Android 런타임 점검 스크립트, Google Play 등록 준비 문서.
...
- `now_app/scripts/check_android_runtime.py` 기준 Flutter, ADB, AVD, 로컬 서버 응답은 확인됐지만 현재 연결된 Android device 상태 기기가 없어 모바일 실제 실행 완료 처리는 보류.
