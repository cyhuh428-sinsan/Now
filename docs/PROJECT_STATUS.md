...
- Web/설치형 화면: 계층형 지식 메모, 일자별 메모 팝업 흐름, Markdown 작성/보기, 가져오기/내보내기, JSON 백업/복원, 검색, 본문 찾기, 탭 편집, 설정, 도움말, 메모 단위 암호화/복호화, 1.2 관계 탐색과 그래프뷰, 1.3 속성 기반 지식 관리, 1.4 Canvas와 시각적 사고 정리, 1.5 첨부/미디어/빠른 기록, 1.6 작성 보조와 명령 체계. 그래프뷰는 전체 그래프, 선택 메모 중심 로컬 그래프, 검색/태그/그룹 필터, 고립 메모, 허브 메모, outgoing links, backlinks, 연결 후보, 그래프 필터 북마크를 제공. 속성 관리는 상태, 중요도, 유형, 출처, 작성자, 마감일, 프로젝트, 속성 목록, 저장 필터, 누락 속성 점검, 템플릿 메모 생성을 제공. Canvas는 메모 카드, 텍스트 카드, 연결선, 확대/축소, 전체 보기, 그래프 주변 메모 초안 생성을 제공. 빠른 기록은 제목 없는 기록 카드, 핀, 색상, 체크리스트, 시간 리마인더, 라벨, 보관함, 첨부 메타, 간단한 그림 저장을 제공. 작성 보조는 명령 팔레트, Slash command, 템플릿 메모, 고유 메모, 랜덤 메모, 합치기/나누기 1차를 제공. Web은 ID/비밀번호 로그인 후 서버 공유 문서만 다루고, 설치형 프로그램은 Electron 로컬 JSON 저장소를 우선 사용하며 PC 로컬 문서와 서버 공유 문서를 함께 다루는 방향으로 분리. 새 지식 메모는 기본 로컬 문서이고, 공유로 선택한 문서만 서버 동기화 대상으로 처리. 화면 언어는 한국어/영어/중국어/일본어/베트남어/아랍어 선택을 지원하고, 추가 언어의 미번역 세부 문구는 영어 fallback으로 표시.
