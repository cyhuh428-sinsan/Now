...
- Web/설치형 화면: 계층형 지식 메모, 일자별 메모 팝업 흐름, Markdown 작성/보기, 가져오기/내보내기, JSON 백업/복원, 검색, 본문 찾기, 탭 편집, 설정, 도움말, 메모 단위 암호화/복호화. Web은 ID/비밀번호 로그인 후 서버 공유 문서만 다루고, 설치형 프로그램은 Electron 로컬 JSON 저장소를 우선 사용하며 PC 로컬 문서와 서버 공유 문서를 함께 다루는 방향으로 분리. 새 지식 메모는 기본 로컬 문서이고, 공유로 선택한 문서만 서버 동기화 대상으로 처리. 화면 언어는 한국어/영어/중국어/일본어/베트남어/아랍어 선택을 지원하고, 추가 언어의 미번역 세부 문구는 영어 fallback으로 표시.
