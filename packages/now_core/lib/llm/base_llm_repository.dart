...
abstract class BaseLlmRepository implements LlmRepository {
  ...
  // 이미지 입력 — 기본은 미지원
  //
  // 받을 수 있는 provider만 아래 둘을 함께 override한다. 기본값을 "미지원"으로
  // 두어야 새 provider가 들어와도 사진이 조용히 버려지지 않는다.
  // ============================================================
  ...
  Future<String> chatWithImage(String prompt, LlmImageInput image) async {
    throw LlmImageUnsupportedException(config.provider);
  }

  /// OpenAI 호환 `/chat/completions`가 받는 이미지 메시지 본문.
  ///
  /// OpenAI, Groq, Ollama가 같은 모양을 쓴다.
  /// 확인한 규격: content 배열에 `{"type":"text"}`와
  /// `{"type":"image_url","image_url":{"url":"data:<mime>;base64,..."}}`를 넣는다.
  List<Map<String, dynamic>> openAiImageContent(
    String prompt,
    LlmImageInput image,
  ) =>
      [
