...
class LocalMeetingRepository {
  ...
  Future<Meeting?> getDailyMemoByDate(DateTime date) async {
    final rows = await getAllMeetings();
    for (final row in rows) {
      final rowDate = row.startedAt ?? row.createdAt;
      if (row.recordType == 'memo' &&
          rowDate.year == date.year &&
  ...
  Future<int> getSegmentCount(String meetingId) async {
    final rows = await getSegments(meetingId);
    return rows.length;
  }
