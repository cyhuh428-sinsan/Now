...
현재 남아 있는 오래된 릴리스 packaged resource에는 과거 백업 규칙이 남아 있을 수 있으므로, 최종 판단은 최신 AAB 재빌드 후 이 점검이 통과하는지를 기준으로 합니다.
...
- 현재 소스의 백업 제외 규칙은 수정되어 있지만, 최종 업로드 전 최신 릴리스 빌드 산출물에서 다시 확인해야 합니다.
