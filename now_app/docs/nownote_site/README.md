...
이 폴더는 `https://nownote.sinsan.kr/privacy`에 표시할 개인정보처리방침 원본 페이지입니다.
현재 1차 구조에서는 별도 정적 사이트 컨테이너가 아니라 NowNote 서버가 이 HTML을 읽어 `/privacy`와 `/privacy-policy`에서 직접 제공합니다.
루트 `https://nownote.sinsan.kr/`는 NowNote Web 프로그램입니다.
...
## 도메인 연결
1차 기본값은 `https://nownote.sinsan.kr/`를 NowNote Web 프로그램으로, `https://nownote.sinsan.kr/privacy`를 개인정보처리방침으로 연결하는 방식입니다.
리버스 프록시는 모든 요청을 NowNote 서버의 `http://127.0.0.1:8750`으로 전달합니다.
