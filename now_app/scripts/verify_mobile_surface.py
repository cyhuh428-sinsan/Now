...
def main() -> None:
    ...
    require_text(runtime_checklist, [("check_android_runtime.py", "runtime checklist Android runtime check command"), ("check_android_emulator.py", "runtime checklist Android emulator check command"), ("check_android_launch.py", "runtime checklist Android launch check command"), ("Android 에뮬레이터", "runtime checklist emulator scope"), ("실제 Android 기기", "runtime checklist physical device scope"), ("홈의 오늘 메모", "runtime checklist home daily memo"), ("같은 메모장에 이어서 저장", "runtime checklist daily append model"), ("3단계 메모 아래에는 더 이상 하위 메모", "runtime checklist tree depth guard"), ("실시간 변환", "runtime checklist realtime voice"), ("녹음 후 변환", "runtime checklist record-then-transcribe"), ("새 지식 메모로 추가", "runtime checklist Markdown import copy model"), ("10.0.2.2", "runtime checklist emulator server URL"), ("2단계 인증 코드는 저장되지 않고", "runtime checklist request-only 2FA")], "runtime checklist", failures)
    require_text(android_runtime_check, [("NowNote Android runtime check", "Android runtime check summary"), ("adb devices -l", "Android runtime check ADB devices"), ("check_android_emulator.py", "Android runtime check emulator next command"), ("flutter run -d", "Android runtime check flutter run command"), ("10.0.2.2", "Android runtime check emulator server URL"), ("--require-server", "Android runtime check server strict option"), ("--require-physical", "Android runtime check physical device strict option")], "Android runtime check", failures)
    require_text(android_emulator_check, [("NowNote Android emulator check", "Android emulator check summary"), ("-list-avds", "Android emulator check AVD listing"), ("--start", "Android emulator check start option"), ("sys.boot_completed", "Android emulator check boot wait"), ("--launch-app", "Android emulator check launch option"), ("check_android_launch.py", "Android emulator check launch integration")], "Android emulator check", failures)
    require_text(android_launch_check, [("NowNote Android install/launch check", "Android launch check summary"), ("adb devices -l", "Android launch check ADB devices"), ("install", "Android launch check install command"), ("monkey", "Android launch check launcher command"), ("pidof", "Android launch check process verification"), ("--require-physical", "Android launch check physical option")], "Android launch check", failures)
