...
const I18N = {
  ko: {
    ...
    "note.labelNote": "메모",
    // #region nownote-only:desktop 트리 공유 아이콘 문구(한국어). Web 트리에는 공유 아이콘이 없다
  ...
  en: {
    ...
    "note.labelNote": "Note",
    // #region nownote-only:desktop 트리 공유 아이콘 문구(영어). Web 트리에는 공유 아이콘이 없다
...
function bindEvents() {
  ...
  // #region nownote-only:desktop Electron 네이티브 메뉴가 보내는 nownote:menu-command 수신. Web에는 네이티브 메뉴가 없다
...
// #region nownote-only:desktop Electron 메뉴 명령(검색 / 본문 찾기) 처리 함수. Web에는 네이티브 메뉴가 없다
...
// #region nownote-only:desktop 트리 노드 공유 상태 아이콘 계산. Web 트리에는 이 아이콘이 없다
...
function treeNodeElement(node) {
  ...
  // #region nownote-only:desktop 공유 아이콘을 포함한 트리 라벨 마크업. Web은 nownote-only:web 쪽을 쓴다
