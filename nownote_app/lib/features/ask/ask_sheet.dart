...
/// 아니라서 대화는 저장하지 않고 메모리에만 둔다. 오늘 메모 화면과 계층
/// 메모 화면이 이 위젯 하나를 함께 쓴다.
///
/// 실제 묻기 동작(길이 상한, 잠긴 메모 차단, 오류 문구)은
/// `package:now_core`의 `AskService`가 다 한다. 이 화면은 그 결과를
/// 보여 주고, 답을 메모에 넣을지·복사할지·버릴지만 다룬다.
///
/// Now의 같은 화면(`now_app/lib/features/ask/ask_sheet.dart`)과 기능은
/// 같지만, NowNote는 다크 모드가 기본 요구사항이라 색은 하드코딩 hex 대신
/// `Theme.of(context).colorScheme`을 쓴다.
library;
...
class AskSheetState extends ConsumerState<AskSheet> {
  ...
  Widget _buildAskTab(ColorScheme colorScheme) {
    return Column(
    ...
    return Center(
      child: Padding(
        ...
        child: Text(
          ...
          style: TextStyle(
              fontSize: 13, color: colorScheme.onSurfaceVariant, height: 1.5),
        ),
      ),
    );
  }
