    ...
    if messenger_api_path.exists():
        ...
        check_text_contains(
            ...
            [
                ...
                ("require_user_api_access", "2.3 messenger accepts Web session or app token", "messenger auth guard"),
                ("X-Now-Web-Session", "2.3 messenger accepts hosted Web session header", "messenger web session header"),
                ("X-Now-User-Token", "2.3 messenger accepts app/desktop user token header", "messenger user token header"),
                ("web session required", "2.3 messenger rejects missing auth headers", "missing messenger auth"),
    ...
    if messenger_smoke_path.exists():
        ...
        check_text_contains(
            ...
            [
                ...
                ("NOW_USER_TOKEN_REQUIRED", "Messenger smoke runs in public token-required mode", "token required smoke mode"),
                ("issue_user_device_access_token", "Messenger smoke issues app/desktop token", "app token smoke setup"),
                ("X-Now-User-Token", "Messenger smoke checks app/desktop token access", "app token messenger access"),
