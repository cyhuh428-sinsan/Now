...
Linux/Docker 배포 순서는 [DEPLOY.md](DEPLOY.md)를 기준으로 빠르게 확인할 수 있습니다.
공용 서버 오픈 절차와 reverse proxy 예시는 [PUBLIC_SERVER.md](PUBLIC_SERVER.md)를 기준으로 확인합니다.
WSL 실행은 개발/테스트용이며, 실제 사용자는 공용 서버를 쓰거나 별도 Linux 서버에 개인 서버를 설치합니다.
...
구형 개인 서버 API 토큰과 DB 비밀번호는 반드시 예시값에서 바꿉니다.
공용 서버 일반 사용자는 `NOW_API_TOKEN`을 입력하지 않습니다. Web은 사용자 ID와 비밀번호로 로그인하고, 설치형 프로그램과 모바일 앱은 서버 주소, 사용자 ID, 앱/설치형 접속 토큰으로 연결합니다.
...
Linux 서버에서 소스 갱신부터 smoke test까지 한 번에 확인하려면 아래 도우미를 사용할 수 있습니다.
...
사용자 직접 가입, Web 로그인, 앱/설치형 연결 토큰 발급/재확인, 2단계 코드 검증 절차, 사용자별 데이터 격리 자동 검증은 smoke test 기준으로 준비 완료 항목에 포함됩니다.
공용 서버에서 비밀번호 재설정을 열려면 SMTP 설정도 함께 완료해야 합니다.
...
`NOW_MESSENGER_STORAGE_DIR`은 메신저 첨부 저장 경로이며, Docker Compose의 `now_messenger_data` 볼륨도 같은 경로에 붙습니다.
메신저 첨부는 `NOW_MESSENGER_MAX_UPLOAD_MB`, `NOW_MESSENGER_ALLOWED_EXTENSIONS`, `NOW_MESSENGER_ALLOWED_MIME_TYPES` 기준으로 크기, 확장자, MIME을 함께 제한합니다. 기본값은 `application/octet-stream`을 허용하지 않으므로, 실제 파일 형식을 확인할 수 없는 업로드는 거부됩니다.
`NOW_USER_TOKEN_REQUIRED=false`는 개인 Docker 서버 기본값입니다.
공용 서버 오픈 전에는 `NOW_USER_TOKEN_REQUIRED=true`로 바꿔 앱/설치형 데이터 API에서 `X-Now-User-Token`을 요구하게 합니다.
공용 서버로 공개할 때는 `NOW_PUBLIC_BASE_URL=https://도메인`과 `NOW_BEHIND_REVERSE_PROXY=true`를 설정합니다.
공용 서버에서 사용자가 비밀번호를 직접 재설정하려면 `NOW_SMTP_HOST`, `NOW_SMTP_FROM`, 필요한 경우 `NOW_SMTP_USERNAME`, `NOW_SMTP_PASSWORD`를 설정합니다.
개인 로컬 서버나 내부망 테스트 서버는 비워두거나 `false`로 둘 수 있습니다.

자체 서버 사용자는 모바일 앱과 설치형 프로그램 설정 화면에 다음 값을 입력합니다.

- 서버 주소: `http://서버주소:8750`
- 사용자 ID: 단독 사용자는 기본 `local_user`, 여러 사용자를 구분할 때는 Web에서 가입했거나 운영자가 정한 owner ID
- 앱/설치형 접속 토큰: `NOW_USER_TOKEN_REQUIRED=true`이면 Web에서 발급한 앱/설치형 연결 토큰 또는 관리자 화면에서 발급한 운영 보조 토큰
- 구형 개인 서버 API 토큰: 2.3 기본 사용 흐름에서는 필요 없으며, 기존 개인 서버 호환이 필요할 때만 고급 설정에서 사용합니다.
- 2단계 인증 코드: 저장하지 않고 연결 확인 때만 입력합니다.

Web 프로그램 사용자는 서버 주소 자체에 접속한 뒤 사용자 ID와 Web 로그인 비밀번호로 로그인합니다.

모바일 앱과 설치형 프로그램은 서버 설정 화면에서 사용자 프로필을 불러오고 이메일, 표시 이름, 시간대를 저장할 수 있습니다.
사용자 그룹, 2단계 인증 사용 여부, 활성 상태는 서버 관리 화면에서 운영자가 관리합니다.
비활성 사용자는 프로필 조회 외의 동기화, 메모, 녹음, 분석 API 사용이 차단됩니다.

설치형 프로그램은 화면 설정의 서버 연결 항목에 값을 저장한 뒤 연결 테스트를 실행합니다.
서버 API는 브라우저 기반 Web 클라이언트가 호출할 수 있도록 CORS를 허용합니다.
공개 운영 시 `NOW_API_TOKEN`은 일반 사용자 입력값이 아니라 운영자/관리자 API 보호와 구형 개인 서버 호환을 위한 서버 환경값으로 설정합니다.
서버 제공 Web은 로그인 직후 서버 공유 문서를 먼저 불러오고, 편집 내용은 서버로 다시 저장합니다.
일자별 메모는 기본 공유 대상으로 저장하고, 지식 메모는 사용자가 공유로 선택한 항목만 `/api/v1/sync`로 전송합니다.
지식 메모는 주제/분류/메모 3단계 구조를 유지해야 하며, 중간 단계 없이 하위 메모만 서버에 저장하는 요청은 거부됩니다.

신산님이 운영하는 공용 NowNote 서버도 같은 서버 프로그램을 사용합니다.
공용 서버의 일반 사용자는 Web에서 직접 가입하고, 앱/설치형 접속 토큰도 Web에서 직접 발급/재확인합니다.
운영자용 `NOW_API_TOKEN`, 관리자 사용자 생성/수정, 관리자 토큰 발급은 운영자 계정 선등록, 테스트 계정, 장애 대응용 보조 기능입니다. 일반 사용자의 기본 연결 안내에는 포함하지 않습니다.
관리 화면의 사용자 그룹, 2단계 인증 사용 여부, 활성 상태와 사용자 토큰 확인 화면/API는 공용 운영을 위한 모니터링 기능입니다.
...
구버전 Docker Compose 환경에서 `docker compose`가 일부 옵션을 인식하지 못하면 `docker-compose` 명령을 사용합니다.
...
`/`는 서버가 제공하는 NowNote Web 프로그램입니다.
`/app/`은 이전 안내와 호환하기 위한 Web 프로그램 보조 주소입니다.
`/privacy`는 Google Play와 사용자 안내용 개인정보처리방침입니다.
...
사용자는 Web에서 직접 가입할 수 있으며, `/admin/users/new`는 운영자 계정 선등록, 테스트 계정, 장애 대응용 보조 화면입니다.
운영자는 사용자 생성/수정 화면에서 Web 로그인 비밀번호를 초기화하거나 사용자 상태를 조정할 수 있습니다.
사용자는 Web 설정 화면에서 앱/설치형 연결 토큰을 직접 발급하고 다시 확인할 수 있습니다.
사용자 수정 화면에서도 운영 보조용 사용자별 접속 토큰을 발급할 수 있습니다.
...
`/admin/ops`는 토큰, DB, 분석 작업, 삭제 표시 메모, 사용자 상태, 비활성 기기, 메신저 첨부 저장소와 첨부 용량 등 운영 점검 항목을 확인하는 읽기 전용 화면입니다.
...
`/admin/release`는 1차 마무리 체크리스트의 완료/남은 항목, 외부 조건별 보류 사유, 다음 행동을 확인하고 남은 항목의 완료 증빙을 바로 저장하는 화면입니다.
`/admin/evidence`는 실제 기기, 공용 서버, Play Console, GitHub Actions, 라이선스처럼 사람이 확인해야 하는 남은 항목의 필요 증빙, 기록 템플릿, 저장 기록, 완료/미기록 집계를 확인하고 증빙 기록을 저장하는 화면입니다.
...
사용자별 접속 토큰 원문, Web 발급 기기별 연결 토큰 원문, 토큰 해시는 백업 JSON에 포함하지 않고, 토큰 발급 여부만 표시합니다.
...
Linux/Docker 배포 갱신 절차는 [DEPLOY.md](DEPLOY.md)를 기준으로 확인합니다.
...
이 API는 `/admin/ops` 화면과 같은 기준으로 DB, 녹음 저장소, 고아 녹음 파일, 누락 녹음 파일, 메신저 첨부 저장소, 메신저 첨부 용량, 누락 메신저 첨부, 토큰, DB 기본 비밀번호, 사용자 상태, 비활성 기기, 분석 작업 상태, 백업/복구 절차 확인 상태를 JSON으로 반환합니다.
...
`/admin/release`의 바로 완료 증빙 기록 또는 `/admin/evidence`에서 같은 항목의 최신 수동 증빙 기록을 `완료`로 저장하면 `summary.evidence_done`에 집계되고 완료 수에도 반영됩니다.
...
이 API는 `/admin/evidence`와 `/admin/release` 화면에서 저장한 수동 증빙 기록을 JSON으로 반환합니다. 같은 경로에 `POST` 요청을 보내면 `group_name`, `section`, `label`, `result`, `checked_by`, `evidence_location`, `actual_note`, `memo` 값을 저장할 수 있습니다.
...
2.3 메신저 구조만 운영 DB를 건드리지 않는 임시 DB/임시 첨부 저장소로 확인할 때:

```powershell
python .\scripts\messenger_smoke_test.py
```

기본 요청 대기 시간은 30초입니다. 요청 대기 시간이 더 필요하면 `--timeout 초`를 추가합니다.
컨테이너가 올라오는 중이면 `--ready-retries 횟수 --ready-delay 초`로 `/health/ready` 준비 대기를 늘릴 수 있습니다.

`NOW_USER_TOKEN_REQUIRED=true`로 공용 서버 모드를 점검할 때 `--user-token`을 생략하면 smoke test가 관리자 토큰으로 검증용 `local_user` 토큰을 자동 발급합니다.

```powershell
python .\scripts\smoke_test.py --base-url http://localhost:8750 --token 긴-랜덤-토큰
```

이미 발급된 사용자별 접속 토큰을 직접 사용할 때:
...
`public_server_readiness`는 공용 서버 오픈 전 준비 상태를 알려줍니다. `ready`에는 서버가 지원하는 사용자 직접 가입, 앱/설치형 연결 토큰, 사용자 프로필 관리, 기기 레지스트리, 사용자별 기기 조회/해제 API, 백업/복구 점검, 사용자별 데이터 격리 자동 검증, 사용자 로그인, 2단계 코드 검증 절차 항목이 들어갑니다. SMTP 설정이 끝나면 이메일 비밀번호 재설정 항목도 `ready`가 됩니다. `NOW_PUBLIC_BASE_URL=https://...`와 `NOW_BEHIND_REVERSE_PROXY=true`가 설정되면 공개 HTTPS/reverse proxy 항목도 `ready`가 되고, 부족하면 `remaining`에 남습니다. `items`는 운영 화면과 같은 기준의 상세 목록입니다.
...
사용자가 Web에서 발급한 앱/설치형 연결 토큰을 브라우저에서 확인하는 화면입니다.
...
성공하면 사용자 프로필과 마지막 로그인 시각을 갱신한 결과를 반환합니다. 응답에는 사용자 ID, 그룹, 활성 상태, 토큰 종류, 기기 ID, 마지막 사용 시각을 확인할 수 있는 `token` 객체도 포함됩니다.
실패하면 `user not found`, `invalid user token`, `user inactive`, `device inactive` 같은 사유를 반환합니다.
...
서버 제공 Web 프로그램이 사용자 ID와 비밀번호로 로그인합니다.
요청 본문은 `owner_id`, `password`, 필요한 경우 `two_factor_code`를 사용합니다.
성공하면 짧은 Web 세션 토큰을 반환하며, Web 프로그램은 이 값을 브라우저 세션에만 보관하고 `X-Now-Web-Session` 헤더로 데이터 API를 호출합니다.

`POST /api/v1/auth/register`

Web 프로그램에서 일반 사용자가 직접 가입합니다.
요청 본문은 `owner_id`, `password`, `email`, 선택적으로 `display_name`, `timezone`, `device_id`, `device_name`을 사용합니다.
성공하면 Web 세션과 앱/설치형 연결 토큰을 함께 반환합니다.

`POST /api/v1/auth/device-token`

Web 세션으로 로그인한 사용자가 앱/설치형 연결 토큰을 발급하거나 재발급합니다.
요청 본문은 `owner_id`, `device_id`, `device_name`을 사용합니다.

`GET /api/v1/auth/device-tokens?owner_id=...`

Web 세션으로 로그인한 사용자가 자신이 발급한 앱/설치형 연결 토큰 목록을 확인합니다.
사용자가 다시 볼 수 있어야 하므로 응답에는 연결 토큰 원문이 포함됩니다. 관리자 export와 백업 JSON에는 포함하지 않습니다.

`POST /api/v1/auth/password-reset/request`

사용자 ID와 등록 이메일이 일치하면 비밀번호 재설정 코드를 이메일로 보냅니다.
공용 서버에서는 `NOW_SMTP_HOST`와 `NOW_SMTP_FROM` 설정이 필요합니다.

`POST /api/v1/auth/password-reset/confirm`

이메일로 받은 재설정 코드와 새 비밀번호를 제출해 비밀번호를 변경합니다.

`GET /api/v1/auth/web-session`

Web 프로그램이 현재 Web 세션이 유효한지 확인합니다.
...
사용자 그룹은 Web에서 관리자 초대코드로 참가할 수 있고, 2단계 인증 사용 여부와 활성 상태는 관리자 화면에서 관리합니다.

`POST /api/v1/users/{owner_id}/group-join`

Web 로그인 사용자가 관리자에게 받은 `group_name`과 `invite_code`로 그룹에 참가합니다.
초대코드가 없는 그룹이나 비활성 그룹에는 참가할 수 없습니다.

`GET /api/v1/users/{owner_id}/groups`

Web 로그인 사용자가 참가 가능한 활성 그룹 목록을 조회합니다.
응답에는 그룹 이름, 설명, 초대코드 설정 여부만 포함하며 초대코드 원문이나 해시는 반환하지 않습니다.
...
관리자가 운영 보조용 사용자별 접속 토큰을 발급합니다.
일반 사용자의 앱/설치형 연결 토큰은 Web에서 직접 발급하는 흐름을 기준으로 합니다.
관리자 발급 토큰은 응답에서 한 번만 확인하는 운영 보조 토큰입니다.
...
단일 메모를 생성하거나 갱신합니다.
공유 문서 일관성을 위해 서버는 먼저 `owner_id + device_id + local_id` 조합을 찾고, 없으면 같은 사용자의 `owner_id + local_id` 문서를 찾아 갱신합니다.
계층 메모는 부모 단계가 정확히 한 단계 위에 존재해야 하며, 중간 단계 없이 하위 메모만 저장하는 요청은 `400`으로 거부됩니다.
...
앱/설치형 요청은 `owner_id`, `device_id`, `X-Now-User-Token` 검증을 분리합니다. 토큰이 없으면 `user token required`, 사용자 ID가 없으면 `user not found`, 토큰이 사용자와 맞지 않으면 `invalid user token`, 기기가 비활성이면 `device inactive`, 기기 ID가 비어 있으면 `device_id required`로 응답합니다.
Web 세션 요청은 `X-Now-Web-Session`으로 검증하며, 그룹 공유 문서 pull 범위는 Web 세션 요청에서만 포함됩니다.
...
같은 그룹의 일부 사용자로 채팅방을 생성합니다. 참여자가 아니면 메시지 조회, 전송, 첨부 다운로드가 `room member required`로 차단됩니다.

`GET /api/v1/messenger/rooms/{room_id}/messages`

참여 중인 채팅방 메시지와 첨부 메타데이터를 조회합니다.

`POST /api/v1/messenger/rooms/{room_id}/messages`

참여 중인 채팅방에 텍스트 메시지를 전송합니다.

`POST /api/v1/messenger/rooms/{room_id}/attachments`

참여 중인 채팅방에 첨부를 업로드합니다. 서버는 크기, 확장자, MIME을 모두 검증하며 기본 정책은 `application/octet-stream`을 허용하지 않습니다.

`GET /api/v1/messenger/attachments/{attachment_id}?owner_id=...`

참여 중인 채팅방의 첨부만 다운로드할 수 있습니다.

`POST /api/v1/messenger/rooms/{room_id}/read`

채팅방의 마지막 읽은 메시지 ID를 저장하고 현재 `unread_count`를 반환합니다.

`GET /api/v1/messenger/policy`

메신저 첨부 크기, 허용 확장자, 허용 MIME, 이미지 확장자 정책을 반환합니다.
