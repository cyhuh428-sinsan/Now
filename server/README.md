...
`public_server_readiness`는 공용 서버 오픈 전 준비 상태를 알려줍니다. `ready`에는 이미 서버가 지원하는 사용자별 접속 토큰, 사용자 프로필 관리, 기기 레지스트리, 백업/복구 점검 항목이 들어가고, `remaining`에는 남은 로그인/토큰 전달 화면, 실제 2단계 인증, 기기 등록/해제 사용자 흐름, 사용자별 데이터 격리 검증, 공개 HTTPS/reverse proxy 항목이 들어갑니다. `items`는 운영 화면과 같은 기준의 상세 목록입니다.
