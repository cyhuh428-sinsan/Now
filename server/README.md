...
- 사용자별 접속 토큰: `NOW_USER_TOKEN_REQUIRED=true`이면 Web에서 발급한 앱/설치형 연결 토큰 또는 관리자 화면에서 발급한 운영 보조 토큰
...
서버 API는 브라우저 기반 Web 클라이언트가 호출할 수 있도록 CORS를 허용하며, 공개 운영 시에는 `NOW_API_TOKEN`을 반드시 설정합니다.
...
공용 서버의 일반 사용자는 Web에서 직접 가입하고, 앱/설치형 연결 토큰도 Web에서 직접 발급/재확인합니다.
운영자용 `NOW_API_TOKEN`, 관리자 사용자 생성/수정, 관리자 토큰 발급은 운영자 계정 선등록, 테스트 계정, 장애 대응용 보조 기능입니다.
