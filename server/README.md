...
공용 운영 서버에도 같은 프로그램을 사용할 예정이지만, 공용 오픈 전에는 사용자별 로그인/토큰 정책을 별도로 완성해야 합니다.
...
다만 현재 구현은 단일 `NOW_API_TOKEN` 기반이므로 공용 서버 정식 오픈 전에는 사용자별 로그인 또는 사용자별 API 토큰을 추가로 확정해야 합니다.
...
`/admin/users`는 시간대, 2단계 인증 사용 여부, 사용자 그룹, 활성 상태, 최근 접속 시간을 확인하고 사용자 정보를 수정하는 관리 화면입니다.
상태, 그룹, 검색어로 사용자 목록을 필터링할 수 있습니다.
...
`/admin/ops`는 토큰, DB, 분석 작업, 삭제 표시 메모, 사용자 상태 등 운영 점검 항목을 확인하는 읽기 전용 화면입니다.
...
현재 이 토큰은 발급/관리 단계이며, 실제 데이터 API의 사용자별 토큰 강제 검증은 다음 인증 단계에서 연결합니다.
