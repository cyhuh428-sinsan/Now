...
`/admin/release`는 1차 마무리 체크리스트의 완료/남은 항목과 외부 조건별 보류 사유를 확인하는 읽기 전용 화면입니다.
...
이 API는 `/admin/release` 화면과 같은 기준으로 전체 체크리스트 완료 수, 영역별 진행률, 실제 기기/공용 서버/Play Console/라이선스처럼 외부 확인이 필요한 남은 항목 유형을 JSON으로 반환합니다.
