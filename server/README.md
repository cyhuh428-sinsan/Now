...
공용 서버 오픈 전에는 사용자별 접속 토큰과 Web 로그인 비밀번호를 준비한 뒤 `NOW_USER_TOKEN_REQUIRED=true`로 바꿔 앱/설치형 데이터 API에서 `X-Now-User-Token`을 요구하게 합니다.
...
- 사용자별 접속 토큰: `NOW_USER_TOKEN_REQUIRED=true`이면 `/admin/users`에서 발급한 토큰
...
다만 현재 구현은 운영자용 `NOW_API_TOKEN`, 앱/설치형 사용자별 접속 토큰, Web ID/password 로그인, 2단계 코드 검증 절차를 기준으로 합니다.
관리 화면의 사용자 그룹, Web 로그인 비밀번호, 2단계 인증 사용 여부, 활성 상태와 사용자 토큰 확인 화면/API는 공용 운영을 위한 준비 기능입니다.
...
운영자는 사용자 생성/수정 화면에서 Web 로그인 비밀번호를 설정할 수 있습니다.
