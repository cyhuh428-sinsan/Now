...
3. 사용자 생성/수정 화면에서 Web 로그인 비밀번호를 설정합니다.
4. `/admin/users`에서 모바일 앱과 설치형 프로그램용 사용자별 접속 토큰을 발급합니다.
5. 발급된 토큰 원문은 한 번만 표시되므로 사용자에게 안전하게 전달합니다.
6. 신규 사용자는 처음에는 테스트 그룹 또는 비활성 상태로 만들고, 연결 확인 후 활성화합니다.
7. 운영자 계정은 2단계 인증을 켭니다.
8. Web 사용자는 `https://nownote.sinsan.kr`에서 사용자 ID와 비밀번호로 로그인합니다.
9. 모바일 앱과 설치형 사용자는 설정 화면에 서버 주소, 사용자 ID, 기기 ID, 사용자별 접속 토큰을 입력합니다.
