...
세 프로그램은 서버에 연결하면 같은 메모를 동기화합니다.
...
- 집 PC 설치형 프로그램, 휴대폰 앱, 외부 PC Web 프로그램이 같은 서버와 동기화됩니다.
- 사용자는 서버를 설치하지 않습니다.
- 운영자가 발급한 사용자 ID, 사용자별 접속 토큰, 필요한 경우 2단계 인증 코드를 입력합니다.
...
- 집 PC 설치형 프로그램, 휴대폰 앱, 외부 PC Web 프로그램이 자기 서버와 동기화됩니다.
...
화면 설정의 서버 연결 항목에서 운영자가 발급한 값을 입력합니다.
...
모바일 앱: 설정 -> 서버 연결
Web 프로그램: https://nownote.sinsan.kr -> 화면 설정 -> 서버 연결
```

연결 후에는 세 프로그램이 같은 서버를 기준으로 동기화됩니다.
...
설치형 프로그램, 모바일 앱, Web 프로그램의 서버 연결 항목에 아래 값을 넣습니다.
...
4. 사용자는 설치형 프로그램, 모바일 앱, Web 프로그램의 서버 연결 항목에 서버 주소, 사용자 ID, 사용자별 접속 토큰을 입력합니다.
