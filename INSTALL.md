...
- 사용자는 서버를 설치하지 않습니다.
- Web은 사용자 ID와 비밀번호로 로그인합니다.
- 설치형 프로그램과 모바일 앱은 운영자가 발급한 사용자 ID, 사용자별 접속 토큰, 필요한 경우 2단계 인증 코드를 입력합니다.
...
운영자가 만든 사용자 ID와 비밀번호를 입력하면 서버에 공유된 내 문서를 불러옵니다.
...
사용자 ID
Web 비밀번호
사용자별 접속 토큰: 설치형 프로그램과 모바일 앱에서 사용
2단계 인증 코드: 필요한 경우
```
...
사용자 ID: local_user 또는 운영자가 만든 사용자 ID
기기 ID: 앱 또는 프로그램에서 자동 생성한 값
사용자별 접속 토큰: 개인 서버 기본값에서는 비워둠
```

개인 서버도 사용자별 접속 토큰을 강제하려면 관리자 화면에서 사용자 토큰을 발급하고 `NOW_USER_TOKEN_REQUIRED=true`를 사용합니다.
...
운영자는 관리자 화면에서 Web 로그인 비밀번호를 설정하고, 설치형 프로그램/모바일 앱용 사용자별 접속 토큰을 발급합니다.

관리 화면:

```text
https://도메인/admin/users/new
```

순서:

1. 사용자 계정을 만듭니다.
2. Web을 쓸 사용자는 사용자 추가/수정 화면에서 비밀번호를 설정합니다.
3. `/admin/users`에서 설치형 프로그램과 모바일 앱용 사용자별 접속 토큰을 발급합니다.
4. 발급된 토큰 원문은 한 번만 보이므로 안전하게 전달합니다.
5. Web 사용자는 서버 주소에서 사용자 ID와 비밀번호로 로그인합니다.
6. 설치형 프로그램과 모바일 앱 사용자는 서버 주소, 사용자 ID, 사용자별 접속 토큰을 입력합니다.
7. 2단계 인증을 쓰는 사용자는 로그인 또는 토큰 확인 때 6자리 코드를 입력합니다.
