...
- 현재 1차의 설치형 프로그램은 Windows `.exe` 또는 `.msi` 설치 파일이 아닙니다.
- 현재 설치형 기준은 `web` 화면을 브라우저의 PWA 설치 기능으로 독립 실행 창처럼 쓰는 방식입니다.
- 나중에 Windows/macOS/Linux 설치 파일이 필요하면 같은 Web 화면을 Tauri 또는 Electron으로 감싸는 방식으로 확장합니다.
...
현재 1차 기준 설치형 프로그램은 PWA 방식입니다.
즉, `web/index.html`과 같은 Web 화면을 `localhost`, `127.0.0.1`, 또는 HTTPS 주소에서 열고 브라우저의 설치 기능으로 독립 창처럼 사용하는 방식입니다.
PWA로 설치한 뒤에도 서버를 직접 설치하지 않고 공용 서버에 접속할 수 있습니다.
입력 위치는 Web 화면과 같은 화면 설정의 서버 연결 항목입니다.
...
일반 사용자는 별도 설치 파일이 제공되면 그 파일을 사용하면 됩니다.
...
`docker compose`가 정상 동작하는 환경이면 `docker-compose` 대신 `docker compose`를 써도 됩니다.
