...
- 2.3 예정: 메신저 파일/사진 전송은 서버 저장소, 업로드 API, 미리보기, 파일 크기 제한, 확장자 제한을 함께 설계한 뒤 진행합니다.
...
- Planned for 2.3: file/photo sending in messenger will be designed together with server storage, upload APIs, previews, file size limits, and extension limits.
...
- 2.3 计划: 消息中的文件/图片发送将与服务器存储、上传 API、预览、文件大小限制和扩展名限制一起设计后推进。
...
- 2.3 予定: メッセンジャーのファイル/写真送信は、サーバーストレージ、アップロード API、プレビュー、ファイルサイズ制限、拡張子制限を一緒に設計してから進めます。
...
- Dự kiến 2.3: gửi file/ảnh trong messenger sẽ được thiết kế cùng với lưu trữ server, API upload, xem trước, giới hạn dung lượng file và giới hạn phần mở rộng.
...
- المخطط في 2.3: سيتم تصميم إرسال الملفات/الصور في المرسال مع تخزين الخادم، وواجهات upload API، والمعاينة، وحدود حجم الملف، وحدود الامتدادات.
