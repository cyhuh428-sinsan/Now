...
- `desktop`: Windows `.exe` installer package. The phase-one artifact wraps the Web screen, while the final desktop program is planned as a separate PC client
...
- `desktop`: Windows `.exe` 安装版封装。第一阶段产物基于 Web 画面封装，最终安装版计划分离为独立 PC 客户端
...
- `desktop`: Windows `.exe` インストール版パッケージ。第一段階の成果物は Web 画面のラップですが、最終的なインストール版は別 PC クライアントとして分離予定
...
- `desktop`: gói cài đặt Windows `.exe`. Sản phẩm giai đoạn một bọc màn hình Web, còn bản cài đặt cuối cùng sẽ tách thành client PC riêng
...
- `desktop`: حزمة تثبيت Windows بصيغة `.exe`. ناتج المرحلة الأولى يلف شاشة Web، أما النسخة النهائية فمخطط لها كعميل PC منفصل
