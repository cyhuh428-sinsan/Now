...
- 2.3 예정: 메신저 파일/사진 전송과 일부 그룹원 채팅방은 서버 저장소, 업로드 API, 미리보기, 파일 크기 제한, 확장자 제한, 참여자 권한을 함께 설계한 뒤 진행합니다.

## 구성

- `now_app`: Flutter 모바일 앱
- `web`: 서버가 루트 주소에서 제공하는 공유 문서 전용 Web 화면
- `desktop`: Windows `.exe` 설치형 프로그램. Web 정적 파일을 포장하지 않는 별도 PC 클라이언트이며, 디자인과 기본 구조는 Web과 같은 계열로 유지
...
- Planned for 2.3: file/photo sending and selected-member chat rooms in messenger will be designed together with server storage, upload APIs, previews, file size limits, extension limits, and participant permissions.

## Structure

- `now_app`: Flutter mobile app
- `web`: server-hosted Web screen for shared documents
- `desktop`: Windows `.exe` installer. It is a separate PC client that does not wrap Web static files, while keeping the same design family and basic structure as Web
...
- 2.3 计划: 消息中的文件/图片发送和部分成员聊天室，将与服务器存储、上传 API、预览、文件大小限制、扩展名限制和参与者权限一起设计后推进。

## 组成

- `now_app`: Flutter 移动应用
- `web`: 由服务器根地址提供的共享文档专用 Web 画面
- `desktop`: Windows `.exe` 安装程序。它是不封装 Web 静态文件的独立 PC 客户端，并保持与 Web 相同的设计风格和基本结构
...
- 2.3 予定: メッセンジャーのファイル/写真送信と一部メンバー向けチャットルームは、サーバーストレージ、アップロード API、プレビュー、ファイルサイズ制限、拡張子制限、参加者権限を一緒に設計してから進めます。

## 構成

- `now_app`: Flutter モバイルアプリ
- `web`: サーバーのルートアドレスで提供される共有文書専用 Web 画面
- `desktop`: Windows `.exe` インストーラー。Web 静的ファイルをラップしない独立 PC クライアントで、Web と同じデザイン系統と基本構造を維持します
...
- Dự kiến 2.3: gửi file/ảnh và phòng chat cho một số thành viên trong messenger sẽ được thiết kế cùng với lưu trữ server, API upload, xem trước, giới hạn dung lượng file, giới hạn phần mở rộng và quyền của người tham gia.

## Cấu trúc

- `now_app`: ứng dụng di động Flutter
- `web`: màn hình Web chuyên dùng cho tài liệu chia sẻ, được server cung cấp tại địa chỉ gốc
- `desktop`: trình cài đặt Windows `.exe`. Đây là client PC riêng, không bọc các tệp tĩnh Web, nhưng giữ cùng phong cách thiết kế và cấu trúc cơ bản với Web
...
- المخطط في 2.3: سيتم تصميم إرسال الملفات/الصور وغرف الدردشة لبعض الأعضاء في المرسال مع تخزين الخادم، وواجهات upload API، والمعاينة، وحدود حجم الملف، وحدود الامتدادات، وصلاحيات المشاركين.

## البنية

- `now_app`: تطبيق Flutter للهاتف
- `web`: شاشة Web مخصصة للمستندات المشتركة يقدمها الخادم من عنوان الجذر
- `desktop`: مثبّت Windows بصيغة `.exe`. إنه عميل PC منفصل لا يغلّف ملفات Web الثابتة، مع الحفاظ على نفس عائلة التصميم والبنية الأساسية مثل Web
