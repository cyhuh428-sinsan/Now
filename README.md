...
모바일 앱은 빠른 기록과 음성 메모를 중심으로 사용하고, Windows 설치형 프로그램과 서버 제공 Web 프로그램은 계층형 지식 메모를 중심으로 사용합니다. 서버는 Docker 기반으로 직접 운영하거나 공용 서버에 연결할 수 있도록 준비합니다.

## 구성

- `now_app`: Flutter 모바일 앱
- `web`: 서버 루트 Web 프로그램과 설치형 프로그램에서 쓰는 Web 화면 원본
- `desktop`: Windows `.exe` 설치형 프로그램 포장
...
- Web 프로그램은 외부 PC에서 `https://nownote.sinsan.kr` 또는 개인 서버 주소로 접속합니다.
